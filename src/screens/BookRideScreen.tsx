...
const BookRideScreen: React.FC = () => {
  ...
  const nycBoundary: L.LatLngExpression[] = [
    ...
    [40.917577, -74.17], // Northwest (moved right to near EWR)
    [40.477399, -74.17], // Close the polygon
  ];
  ...
  useEffect(() => {
    if (mapInstanceRef.current && boundaryRef.current) {
      ...
      const handleMapClick = (e: L.LeafletMouseEvent) => {
        ...
        if (!isInBounds) {
          alert(
            "Please select a location within the NYC area (highlighted in purple).",
          );
          return;
        }
  ...
  const handleBookRide = async () => {
    if (!pickupLocation || !dropoffLocation || !selectedTime) {
      alert(
        "Please select pickup location, dropoff location, and pickup time.",
      );
      return;
    }
  };
    ...
    <div className="min-h-screen bg-pink-gradient">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white rounded-xl p-4 mb-6 shadow-lg">
          <h2 className="text-lg font-semibold text-deep-violet mb-4">
            Select Locations
          </h2>
        ...
        {/* Time Selection */}
        <div className="bg-white rounded-xl p-4 mb-6 shadow-lg">
          <h3 className="text-lg font-semibold text-deep-violet mb-4">
            Select Pickup Time
          </h3>
